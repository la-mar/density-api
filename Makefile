...
run-tests:
	pytest src/density tests/

recreate-and-seed-db:
	poetry shell && density db recreate && seed_db

cov:
	export CI=false && pytest -x --cov src/density tests/ --cov-report html:./coverage/coverage.html --log-cli-level 30 --log-level 20 -vv

cicov:
	export CI=true && pytest -x --cov src/density tests/ --cov-report html:./coverage/coverage.html --log-cli-level 10 --log-level 10 -vv

ncov:
	# generate cov report to stdout
	pytest --cov src/density tests/
