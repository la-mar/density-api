...
def init(dir: Path = Path("src/density/db/migrations"), args: List[str] = None):
...
cli = click.Group(help="Density API")
...
def web(args):  # nocover
    cmd = ["uvicorn", "density.main:app"] + list(args)
    subprocess.call(cmd)
...
def dev(args):  # nocover
    cmd = ["uvicorn", "density.main:app", "--reload"] + list(args)
    subprocess.call(cmd)
