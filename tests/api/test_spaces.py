...
from api.v1.endpoints.spaces import ERROR_404
...
path: str = "/api/v1/spaces/"


@pytest.fixture(autouse=True)
async def seed_spaces(bind):
    for x in range(0, 30):
...
class TestEndpoint:
    ...
    async def test_get_space(self, client):
        id = 20
    ...
    async def test_update_exising_space(self, client):
        id = 10
    ...
    async def test_partial_update_exising_space(self, client):
        id = 10
        name = rand_str(length=25)
    ...
    async def test_delete_space_not_found(self, client):
        id = 99999
        response = await client.delete(f"{path}{id}")
        assert response.status_code == codes.HTTP_404_NOT_FOUND
        data = response.json()
