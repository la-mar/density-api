...
@pytest.fixture
def records(ids):
    yield [{"id": i, "name": testutils.rand_str()} for i in ids]
...
class TestBulkIO:
    ...
    async def test_bulk_upsert_update_on_conflict(
        ...
        results = await Model.query.gino.load((Model.id, Model.name)).all()
        expected = [(d["id"], d["name"]) for d in records2]
    ...
    async def test_bulk_upsert_ignore_on_conflict(
        ...
        results = await Model.query.gino.load((Model.id, Model.name)).all()
        expected = [(d["id"], d["name"]) for d in records]
        assert results == expected
    ...
    async def test_bulk_upsert_handle_data_error(self, bind, caplog, records, ids):
        ...
        records.append({"id": 99999999999999999999, "name": testutils.rand_str()})
    ...
    async def test_bulk_upsert_handle_generic_error(self, bind, caplog, records):
        ...
        with pytest.raises(Exception):
            records = [{"name": testutils.rand_str()}, {}]
            await Model.bulk_upsert(records)
