...
class TestMisc:
    def test_import_as_module(self):
        """ simulate executing the module from the command line
            i.e. python -m density """
        import density.__main__  # noqa
...
class TestCLISlow:
    def test_run_web(self, capfd):
        autokill_subprocess(
            "density", "run", "web", "--port", str(get_open_port()), delay=5
        )
        captured = capfd.readouterr()
    ...
    def test_launch_dev_server(self, capfd):
        autokill_subprocess(
            "density", "run", "dev", "--port", str(get_open_port()), delay=5
        )
        captured = capfd.readouterr()
        assert "Uvicorn running" in captured.err
    ...
    def test_run_cli(self, capfd):
        autokill_subprocess("density", delay=10)
        captured = capfd.readouterr()
